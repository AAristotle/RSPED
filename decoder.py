...
#         self.fc = torch.nn.Linear(embedding_dim * channels, embedding_dim)
#
#     def forward(self, embedding, emb_rel, emb_time, triplets, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None, one_hot_s_freq = None, s_history_tag = None):
#         e1_embedded_all = F.tanh(embedding)
#         batch_size = len(triplets)
...
#         # emb_time_1 = emb_time_1.unsqueeze(1)
#         # emb_time_2 = emb_time_2.unsqueeze(1)
#         if one_hot_s_freq is not None:
#             one_hot_s_freq = one_hot_s_freq.squeeze(0)
#             s_history_tag = s_history_tag.squeeze(0)
#             # one_hot_o_freq = one_hot_o_freq[triplets[:, 0]]
#             # s_history_tag = s_history_tag[triplets[:, 0]]
#             # frequency = torch.concat([one_hot_s_freq.unsqueeze(0), s_history_tag.unsqueeze(0)], 0)
...
#         elif one_hot_s_freq is None and partial_embeding is not None:
#             x = torch.mm(x, e1_embedded_all.transpose(1, 0))
#             x = torch.mul(x, partial_embeding)
#         else:
#             x = torch.mm(x, e1_embedded_all.transpose(1, 0))+ one_hot_s_freq
#             x = torch.mul(x, s_history_tag)
#         return x
#
...
class TimeConvTransE(torch.nn.Module):
    ...
    def forward(self, embedding, eid, emb_rel, emb_time, nodes_id=None, mode="train", negative_rate=0, partial_embeding=None):
        ...
        time_emb1, time_emb2 = emb_time
        time_emb1 = time_emb1.unsqueeze(1)
        time_emb2 = time_emb2.unsqueeze(1)
        stacked_inputs = torch.cat([e1_embedded, rel_embedded, time_emb1, time_emb2], 1)  # batch_size,2,h_dim
        stacked_inputs = self.bn0(stacked_inputs)  # batch_size,2,h_dim
        x = self.inp_drop(stacked_inputs)  # batch_size,2,h_dim
        x = self.conv1(x)  # batch_size,2,h_dim
        x = self.bn1(x)  # batch_size,channels,h_dim
        ...
        if partial_embeding is None:
            ...
            x = torch.mm(x, e1_embedded_all.transpose(1, 0)) + partial_embeding
            # x = torch.mul(x, partial_embeding)
        return x